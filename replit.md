...
- Telemetry maps onto the schema: `players.war` ← war, `players.twar` ← twar, `players.par` ← par, `players.playerValue` ← player value (often null), plus tier/pct; each flattened grade metric becomes a `player_grades` row with a label/category from `GRADE_META`. These grade columns/tables are the source for `ingestTelemetry`, which projects them into `player_stats` (source `telemetry`) so Telemetry shows up as a raw source in the UI (see the two-data-layers note above). The dashboard/explorer/team lists still rank by snaps (no WAR column in those lists); Telemetry grades are surfaced via the per-source Telemetry tab + the Stats Explorer.
